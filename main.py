...
def setup_dex_client():
    ...
    # Create gRPC channel
    channel = grpc.secure_channel('dex.example.com:5557', credentials)
    
    return api_pb2_grpc.DexStub(channel)

def get_open_merge_requests(gitlab_url, gitlab_token, project_id):
...
def process_merge_requests(gitlab_url, gitlab_token, project_id, dex_client, known_mr_ids):
    """Process all open merge requests and track which ones we've seen"""
    try:
        # Get open merge requests from GitLab
        mrs = get_open_merge_requests(gitlab_url, gitlab_token, project_id)
...
def main():
    ...
    try:
        while True:
            ...
            known_mr_ids, new_mrs = process_merge_requests(gitlab_url, gitlab_token, project_id, dex_client, known_mr_ids)
